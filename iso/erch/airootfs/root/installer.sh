...
# E&E Distro Installer — TUI-based Arch installer with gum
# Boots from custom ISO, partitions disk, installs Arch, clones erch, runs boot.sh
...
ENCRYPTION="false"
DISTRO="erch"
REPO_URL="https://github.com/eande-uk/erch.git"
REPO_BRANCH="master"
...
header() {
  ...
  gum style \
    ...
    --width 60 \
    --padding "1 2" \
    "E&E Distro Installer" \
    "" \
    "This will install ${DISTRO} to your hard drive." \
    "" \
    "Requirements:" \
    "  - Backed up data" \
    "  - Internet connection" \
...
get_disks() {
  ...
  while IFS= read -r line; do
    ...
    disks+=("/dev/${disk} (${size})")
  done < <(lsblk -dno NAME,SIZE,TYPE | grep disk | awk '{print $1, $2, $3}')
  echo "${disks[@]}"
}
...
setup_system() {
  ...
  # Hostname
  HOSTNAME=$(gum input --header "Enter hostname:" --value "erch")
  [[ -z "$HOSTNAME" ]] && HOSTNAME="erch"

  gum_style_properties() {
...
clone_distro() {
  ...
  gum spin --spinner dot --title "Cloning $DISTRO repo..." -- \
    bash -c "
      arch-chroot /mnt bash -c '
...
run_distro_installer() {
  log "Running $DISTRO installer"

  gum spin --spinner dot --title "Running $DISTRO installer..." -- \
    bash -c "
      arch-chroot /mnt bash -c '
        export ${DISTRO^^}_PATH=\"/root/${DISTRO}\"
        cd /root/${DISTRO}
        chmod +x install.sh
        ./install.sh
      '
